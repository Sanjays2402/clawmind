...
export function doctorCommand() {
    return new Command('doctor')
        ...
        if (!res.ok)
            throw new Error(`GET /v1/doctor -> ${res.status}: ${await res.text()}`);
        ...
        process.stdout.write(kleur.gray(`  manifest ${r.counts.manifestDocs} docs / ${r.counts.manifestChunks} chunks  ` +
        ...
        for (const f of r.findings) {
            ...
            if (f.hint)
                process.stdout.write(kleur.gray(`        ${f.hint}\n`));
        }
        if (!r.ok)
            process.exitCode = 1;
    });
