...
export function feedbackCommand() {
    ...
    cmd.command('up <path>')
        ...
        process.stdout.write(kleur.green(`+1 ${path} (boost ${out.boost.toFixed(2)})\n`));
    });
    cmd.command('down <path>')
        .description('Downvote a source path so retrieval ranks it lower')
        .action(async (path) => {
        const out = await apiFetch('POST', '/v1/feedback', { path, vote: -1 });
        process.stdout.write(kleur.yellow(`-1 ${path} (boost ${out.boost.toFixed(2)})\n`));
    });
    ...
    cmd.command('list')
        ...
        .action(async () => {
        const out = (await apiFetch('GET', '/v1/feedback'));
        if (out.items.length === 0) {
            process.stdout.write(kleur.gray('no feedback yet\n'));
            return;
        }
